...
def all_labels(path):
	list_stems = list_text_files(path)
	labels = []
	for name in list_stems:
...
list_of_labeled_envs = ['lemma', 'proposition', 'theorem', 'remark', 'remarks',
	'example', 'exercise', 'situation', 'equation', 'definition']

# Standard labels
list_of_standard_labels = ['definition', 'lemma', 'proposition', 'theorem',
	'remark', 'remarks', 'example', 'exercise', 'situation', 'equation',
...
def list_text_files(path):
	...
	while line.find("\\") >= 0:
		line = line.rstrip()
		line = line.rstrip("\\")
		list_stems = list_stems + " " + line
		line = Makefile_file.next()
	Makefile_file.close()
	list_stems = list_stems + " " + line
	list_stems = list_stems.replace("LIST_STEMS = ", "")
	list_stems = list_stems + " fdl"
	return list_stems.split()
